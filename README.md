...
这是一个面向 MIDI 旋律续写的音乐生成 Agent。项目从早期 notebook / demo 原型升级成了一个可运行、可测试、可解释的工程版本：它会从本地 MIDI 语料中抽取旋律短语，基于音乐特征做检索增强，然后生成一段风格连续的 MIDI 续写，并输出可量化的音乐性评估指标。

我没有把它包装成“万能 AI 作曲神器”，而是保留了比较真实的学生项目演进痕迹：先从符号音乐和旋律续写这个边界清楚的问题做深，把数据流、检索、生成和评估闭环打通。
